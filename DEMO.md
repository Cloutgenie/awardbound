...
A hosted preview is not published from this environment (Vercel is not authenticated). After merge, deploy the `dist` output as a static SPA, or run locally:
...
1. **Landing (`/`)** — Read the hero: “Upload the NOA. Get the calendar.” Confirm five benefit blocks and the fee schedule: Starter **$6,000/yr**, Growth **$15,000/yr**, Institution **$30–50k/yr**. Click **Open director desk**.
...
5. **Grant desk (`/app/grants/awardbound-aamu-t3-2024`)** — Confirm the Sample chip, objectives with owners and due dates, evidence slots (some missing), drawdown vs. award, and upcoming reports. Gold appears only on at-risk dates.
6. **Alerts (`/app/alerts`)** — At least three: objective behind (STEM capacity), missing evidence, report due soon (quarterly memorandum).
7. **Calendar (`/app/calendar`)** — The same dates on one register.
8. **Settings (`/app/settings`)** — Mock identity and **Clear the local register** if you want to start over.
